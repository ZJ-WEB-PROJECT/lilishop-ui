      ...
      <Row @keydown.enter.native="handleSearch">
        <Form ref="searchForm" :model="searchForm" inline :label-width="80" class="search-form">
          <Form-item label="试穿员ID" prop="toMemberId">
            <Input type="text" v-model="searchForm.memberId" placeholder="请输入试穿员ID" clearable style="width: 160px" />
          </Form-item>
          <Form-item label="试穿员名称" prop="toMemberName">
            <Input type="text" v-model="searchForm.memberName" placeholder="请输入试穿员名称" clearable style="width: 160px" />
          </Form-item>
...
import {getMemberIncomeList} from "@/api/im.js";
...
export default {
      ...
      columns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            return h("img", {
              attrs: {
                src: params.row.storeLogo || require('@/assets/emptyImg.png'),
                alt: ""
              },
              style: {
                width: "30px",
  ...
  methods: {
    ...
    //查询打赏记录列表
    getData() {
      API_ChatGift.getMemberIncomeList(this.searchForm).then((res) => {
